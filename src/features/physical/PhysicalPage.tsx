...
import PhysicalStatSection from './components/PhysicalStatSection';
import PhysicalFormDialog from './components/PhysicalFormDialog';
import PhysicalListDialog from './components/PhysicalListDialog';
...
import { usePhysicalItems } from './hooks';
...
export default function PhysicalPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalDailyCost = items.reduce(
      (sum, item) => sum + calcCostPerDay(item.price, item.purchaseDate),
      0,
    );
    return { totalCount, totalValue, totalDailyCost };
  }, [items]);
    ...
    <div className="bg-[#F2F2F7]">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-5 md:py-6 pb-24">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          ...
          <Button
            size="sm"
            className="w-auto self-start bg-[#007AFF] hover:bg-[#0066CC] shadow-sm"
